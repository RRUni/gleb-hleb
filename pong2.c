...
void pole(int sc1, int sc2, int xs, int ys, int y_rocket1, int y_rocket2)
{
    ...
    for (int y = 0; y < HEIGHT; y++)
    {
        ...
        for (int x = 0; x < WIDTH; x++)
        {
            ...
            char symb;
            if ((x == 1) && (y_rocket1 <= y) && (y_rocket1 + 2 >= y))
            {
                printf("|");
            }
            else if ((x == WIDTH - 2) && (y_rocket2 <= y) && (y_rocket2 + 2 >= y))
            {
                printf("|");
            }
            else if ((x == xs) && (y == ys))
...
{
    if ( xs == 1  && ( ys >= y_rocket1 && ys <= y_rocket1 + 2 )) {
        dx = 1;
    }
    if (xs == WIDTH - 2 && (ys >= y_rocket2 && ys <= y_rocket2 + 2)) {
        dx = -1;
    }
    return dx;
}
